    ...
    <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
      <Link href={`/products/${product.slug}`}>
        ...
        >
          ...
          <div className="p-4">
            {product.brand && (
              <p className="text-[10px] font-semibold uppercase tracking-wider mb-1 text-[#EB9220]">
                {product.brand}
              </p>
            )}
            <h3 className="font-semibold text-[#001c54] text-sm leading-tight mb-1 line-clamp-2">{product.name}</h3>
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="text-lg font-bold text-[#001c54]">₹{Number(product.sellingPrice).toFixed(2)}</span>
                {discount > 0 && (
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-bold text-white transition-all hover:scale-105 active:scale-95 bg-[#EB9220] hover:bg-[#D84315]"
              >
