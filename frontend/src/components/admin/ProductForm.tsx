    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="card md:col-span-2">
          <h2 className="font-bold mb-4" style={{ color: '#F8F8F8' }}>Basic Information</h2>
        ...
        {/* Pricing */}
        <div className="card">
          <h2 className="font-bold mb-4" style={{ color: '#F8F8F8' }}>Pricing &amp; GST</h2>
        ...
        {/* Flags */}
        <div className="card">
          <h2 className="font-bold mb-4" style={{ color: '#F8F8F8' }}>Visibility</h2>
          <div className="space-y-4">
            {[["isActive", "Active (visible on store)"], ["isFeatured", "Featured on homepage"]].map(([k, label]) => (
              <label key={k} className="flex items-center justify-between cursor-pointer">
                <span className="text-sm" style={{ color: '#F8F8F8' }}>{label}</span>
                <button type="button" onClick={() => setForm(f => ({ ...f, [k]: !(f as any)[k] }))}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${(form as any)[k] ? "bg-[#FF5C00]" : "bg-[#0C1E39]"}`}>
          ...
          <div className="border-t mt-4 pt-4" style={{ borderColor: '#0C1E39' }}>
            <h3 className="text-sm font-bold mb-3" style={{ color: '#F8F8F8' }}>Short Description</h3>
        ...
        {/* Description */}
        <div className="card md:col-span-2">
          <h2 className="font-bold mb-4" style={{ color: '#F8F8F8' }}>Description</h2>
        ...
        {/* Images */}
        <div className="card md:col-span-2">
          <h2 className="font-bold mb-4" style={{ color: '#F8F8F8' }}>Product Images</h2>
          <div className="flex flex-wrap gap-3 mb-3">
            {existingImages.map(img => (
              <div key={img.id} className="relative h-20 w-20 rounded-xl overflow-hidden" style={{ border: '1.5px solid #0C1E39' }}>
                ...
                <button type="button" onClick={async () => {
                  ...
                  className="absolute top-1 right-1 h-5 w-5 rounded-full flex items-center justify-center hover:bg-red-500/80 transition-colors" style={{ background: '#0C1E39', color: '#F8F8F8' }}>
                  <X size={10} />
                </button>
              </div>
            ))}
            ...
            <label className="h-20 w-20 rounded-xl flex flex-col items-center justify-center cursor-pointer transition-colors" style={{ border: '2px dashed #FF5C00', background: 'rgba(255,92,0,0.05)' }}>
              ...
              <span className="text-[10px]" style={{ color: '#F8F8F8' }}>Upload</span>
        ...
        <div className="card md:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold" style={{ color: '#F8F8F8' }}>Variants (optional)</h2>
          ...
          {variants.length === 0 && <p className="text-sm" style={{ color: '#F8F8F8' }}>No variants — single SKU product.</p>}
          {variants.map((v, i) => (
        ...
        {/* SEO */}
        <div className="card md:col-span-2">
          <h2 className="font-bold mb-4" style={{ color: '#F8F8F8' }}>SEO (optional)</h2>
          <div className="space-y-3">
            <div>
