  ...
  return (
    <main className="relative min-h-screen bg-[#050505] flex items-center justify-center px-6">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -top-40 left-1/4 h-[500px] w-[500px] rounded-full bg-pink-500/10 blur-[120px]" />
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative w-full max-w-md">
        ...
        <div className="card p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="text-right mt-1">
                <Link href="/forgot-password" className="text-xs text-pink-400 hover:text-pink-300">Forgot password?</Link>
              </div>
            </div>

            <motion.button type="submit" disabled={loading} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className="btn-primary w-full py-3 mt-2 disabled:opacity-50 disabled:cursor-not-allowed">
              {loading ? <span className="flex items-center justify-center gap-2"><span className="h-4 w-4 rounded-full border-2 border-white border-t-transparent animate-spin" />Signing in...</span> : "Sign In"}
            </motion.button>
          </form>
          ...
          <p className="text-center text-sm text-white/40">
            ...
            <Link href="/register" className="font-semibold text-pink-400 hover:text-pink-300">Create one free</Link>
          </p>
          <p className="text-center text-sm text-white/40 mt-2">
            Admin?{" "}
            <Link href="/admin/login" className="font-semibold text-white/60 hover:text-white">Admin login →</Link>
          </p>
        </div>
      </motion.div>
    </main>
