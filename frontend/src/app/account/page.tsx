    ...
    <main className="min-h-screen bg-[#FCFAF6]">
      ...
      <div className="pt-24 pb-16 px-6 mx-auto max-w-6xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-black text-[#002A30]">My Account</h1>
          ...
          <aside className="lg:col-span-1">
            <div className="card space-y-1">
              <div className="flex items-center gap-3 pb-4 mb-2 border-b border-[#E8E2D9]">
                ...
                <div>
                  <p className="font-bold text-[#002A30] text-sm">{user?.name}</p>
                  <p className="text-xs text-[#45353E]">Customer</p>
                </div>
              </div>
              {TABS.map((t) => (
                <button key={t.id} onClick={() => setTab(t.id)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${tab === t.id ? "bg-[#48C062]/20 text-[#48C062]" : "text-[#45353E] hover:text-[#002A30] hover:bg-[#FCFAF6]"}`}>
          ...
          <div className="lg:col-span-3">
            ...
            {tab === "profile" && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card">
                <h2 className="font-bold text-[#002A30] mb-6">Profile Information</h2>
            ...
            {tab === "orders" && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                <h2 className="font-bold text-[#002A30] mb-4">Order History</h2>
                {orders.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {orders.map(order => (
                      ...
                      <div className="card hover:border-[#48C062] hover:shadow-md transition-all cursor-pointer">
                          ...
                          <div className="text-right flex flex-col items-end gap-1">
                            ...
                            {order.invoice && (
                              <button
                                ...
                                className="flex items-center gap-1 text-xs text-[#48C062] hover:text-[#48C062] mt-1"
                              >
            ...
            {tab === "addresses" && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-bold text-[#002A30]">Saved Addresses</h2>
                ...
                {addingAddr && (
                  <div className="card mb-4 space-y-3">
                    <h3 className="font-semibold text-[#002A30] text-sm">New Address</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {addresses.map(addr => (
                    <div key={addr.id} className="card relative group">
                      ...
                      <p className="font-bold text-[#002A30] text-sm">{addr.fullName}</p>
...
export default function AccountPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#FCFAF6] flex items-center justify-center"><div className="h-8 w-8 rounded-full border-2 border-[#48C062]/40 border-t-[#48C062] animate-spin" /></div>}>
      <AccountPageContent />
    </Suspense>
  );
}
