  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-3xl font-black text-[#1D3557]">Invoices</h1><p className="text-[#4A6A82] text-sm mt-1">GST Tax Invoices</p></div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? Array.from({ length: 8 }).map((_, i) => <tr key={i}><td colSpan={9} className="px-4 py-3"><div className="h-4 rounded bg-[#F1FAFF] animate-pulse" /></td></tr>)
                : invoices.map((inv: any) => (
                <tr key={inv.id} className="hover:bg-[#F1FAFF] transition-colors">
                  <td className="px-4 py-3 font-mono text-[#1D3557] font-bold text-xs">{inv.invoiceNumber}</td>
                  <td className="px-4 py-3 font-mono text-[#4A6A82] text-xs">{inv.order?.orderNumber}</td>
                  <td className="px-4 py-3 text-[#4A6A82]">{inv.order?.user?.name || "—"}</td>
                  <td className="px-4 py-3 font-bold text-[#1D3557]">₹{Number(inv.totalAmount).toFixed(2)}</td>
                  <td className="px-4 py-3 text-[#4A6A82]">₹{(Number(inv.cgstAmount) + Number(inv.sgstAmount) + Number(inv.igstAmount)).toFixed(2)}</td>
