  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-display font-black text-[#111827]">Categories</h1>
      ...
      <div className="card p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {loading ? Array.from({ length: 6 }).map((_, i) => <tr key={i}><td colSpan={5} className="px-4 py-3"><div className="h-4 rounded bg-[#F4F6FA] animate-pulse" /></td></tr>)
              : roots.map(cat => (
              <>
                <tr key={cat.id} className="hover:bg-[#F4F6FA] transition-colors font-semibold">
                  <td className="px-4 py-3 text-[#111827]">{cat.name}</td>
                  <td className="px-4 py-3 text-[#6B7280]">—</td>
                  <td className="px-4 py-3 text-[#374151]">{cat._count?.products ?? 0}</td>
                  ...
                  <td className="px-4 py-3 flex gap-1">
                    <button onClick={() => openEdit(cat)} className="h-8 w-8 flex items-center justify-center rounded-lg text-[#6B7280] hover:text-blue-400 hover:bg-blue-400/10 transition-all"><Edit3 size={14} /></button>
                    <button onClick={() => handleDelete(cat.id)} className="h-8 w-8 flex items-center justify-center rounded-lg text-[#6B7280] hover:text-red-400 hover:bg-red-400/10 transition-all"><Trash2 size={14} /></button>
                  </td>
                </tr>
                {children(cat.id).map(child => (
                  <tr key={child.id} className="hover:bg-[#F4F6FA] transition-colors">
                    <td className="px-4 py-3 pl-10 text-[#111827]">↳ {child.name}</td>
                    <td className="px-4 py-3 text-[#6B7280]">{cat.name}</td>
                    <td className="px-4 py-3 text-[#374151]">{child._count?.products ?? 0}</td>
                    ...
                    <td className="px-4 py-3 flex gap-1">
                      <button onClick={() => openEdit(child)} className="h-8 w-8 flex items-center justify-center rounded-lg text-[#6B7280] hover:text-blue-400 hover:bg-blue-400/10 transition-all"><Edit3 size={14} /></button>
                      <button onClick={() => handleDelete(child.id)} className="h-8 w-8 flex items-center justify-center rounded-lg text-[#6B7280] hover:text-red-400 hover:bg-red-400/10 transition-all"><Trash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </>
      ...
      {modal !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/60 backdrop-blur-sm px-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md card p-6">
            <h2 className="font-display font-bold text-[#111827] mb-5">{modal?.id ? "Edit" : "New"} Category</h2>
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <button type="button" onClick={() => setForm(f => ({ ...f, isActive: !f.isActive }))}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${form.isActive ? "bg-[#F47C41]" : "bg-[#FFFFFF]"}`}>
