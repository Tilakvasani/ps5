    ...
    <div className="flex h-screen bg-[#FCFAF6] overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 flex-shrink-0 border-r border-[#E8E2D9] bg-white  flex flex-col transition-transform duration-300
        ...
        <div className="flex items-center gap-1 px-5 py-5 border-b border-[#E8E2D9]">
          ...
          <button className="lg:hidden text-[#45353E] hover:text-[#0B1B3D] ml-1" onClick={() => setSidebarOpen(false)}><X size={18} /></button>
        </div>
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-all group ${
                  active ? "bg-[#EB9220]/20 text-[#EB9220]" : "text-[#45353E] hover:text-[#0B1B3D] hover:bg-[#FCFAF6]"}`}>
                <item.icon size={16} className={active ? "text-[#EB9220]" : "text-[#45353E] group-hover:text-[#0B1B3D]"} />
                {item.label}
        ...
        <div className="border-t border-[#E8E2D9] p-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-[#0B1B3D] truncate">{adminName}</p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center gap-4 border-b border-[#E8E2D9] bg-white  px-6 py-3 flex-shrink-0">
          <button className="lg:hidden text-[#45353E] hover:text-[#0B1B3D]" onClick={() => setSidebarOpen(true)}>
            <Menu size={22} />
          </button>
          <div className="flex-1" />
          <Link href="/" target="_blank" className="text-xs text-[#45353E] hover:text-[#0B1B3D] transition-colors border border-[#E8E2D9] rounded-lg px-3 py-1.5">
            View Store ↗
          </Link>
          <Link href="/admin/notifications">
            <div className="relative p-2 text-[#45353E] hover:text-[#0B1B3D] cursor-pointer">
              <Bell size={18} />
