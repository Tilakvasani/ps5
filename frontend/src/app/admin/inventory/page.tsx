  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-display font-black text-white">Inventory</h1>
          <p className="text-white/40 text-sm mt-1">{inventory.length} SKUs tracked</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? Array.from({ length: 10 }).map((_, i) => (
                <tr key={i}><td colSpan={7} className="px-4 py-3"><div className="h-4 rounded bg-white/5 animate-pulse" /></td></tr>
              )) : filtered.map(item => {
                const available = item.qtyInStock - item.reservedQty;
                const isLow = item.qtyInStock <= item.lowStockThreshold;
                return (
                  <tr key={item.id} className={`hover:bg-white/5 transition-colors ${isLow ? "bg-yellow-500/5" : ""}`}>
                    <td className="px-4 py-3">
                      <p className="font-semibold text-white line-clamp-1 max-w-[180px]">{item.product?.name}</p>
                    </td>
                    <td className="px-4 py-3 font-mono text-white/50 text-xs">
                      {item.variant ? item.variant.sku : item.product?.sku}
                      {item.variant && <p className="text-white/30">{item.variant.variantName}</p>}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`font-bold text-lg ${isLow ? "text-yellow-400" : "text-white"}`}>{item.qtyInStock}</span>
                    </td>
                    <td className="px-4 py-3 text-white/50">{item.reservedQty}</td>
                    <td className="px-4 py-3">
                      <span className={`badge ${available > 0 ? "badge-success" : "badge-danger"}`}>{available}</span>
                    </td>
                    <td className="px-4 py-3 text-white/40">{item.lowStockThreshold}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => setMovementModal(item)}
                        className="flex items-center gap-1.5 text-xs font-semibold text-pink-400 hover:text-pink-300 border border-pink-500/20 hover:border-pink-500/40 px-3 py-1.5 rounded-lg transition-all">
                        <Plus size={12} /> Adjust
                      </button>
                    </td>
                  </tr>
      ...
      {movementModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md card p-6">
            <h2 className="font-display font-bold text-white mb-1">Adjust Stock</h2>
            <p className="text-sm text-white/40 mb-5">{movementModal.product?.name}{movementModal.variant ? ` — ${movementModal.variant.variantName}` : ""}</p>
