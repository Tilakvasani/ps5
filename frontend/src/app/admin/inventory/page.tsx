  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-black text-[#002A30]">Inventory</h1>
                ...
                return (
                  <tr key={item.id} className={`hover:bg-[#FCFAF6] transition-colors ${isLow ? "bg-yellow-500/5" : ""}`}>
                    <td className="px-4 py-3">
                      <p className="font-semibold text-[#002A30] line-clamp-1 max-w-[180px]">{item.product?.name}</p>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`font-bold text-lg ${isLow ? "text-yellow-400" : "text-[#002A30]"}`}>{item.qtyInStock}</span>
                    </td>
      ...
      {movementModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white  px-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md card p-6">
            <h2 className="font-bold text-[#002A30] mb-1">Adjust Stock</h2>
