  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-3xl font-display font-black text-white">Users</h1><p className="text-white/40 text-sm mt-1">{total} registered users</p></div>
      </div>
      ...
      <div className="card p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {loading ? Array.from({ length: 8 }).map((_, i) => <tr key={i}><td colSpan={6} className="px-4 py-3"><div className="h-4 rounded bg-white/5 animate-pulse" /></td></tr>)
              : users.map(u => (
              <tr key={u.id} className="hover:bg-white/5 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="h-8 w-8 rounded-full bg-gradient-to-br from-pink-500 to-yellow-400 flex items-center justify-center text-xs font-black text-black flex-shrink-0">
                      {u.name?.[0]?.toUpperCase()}
                    </div>
                    <div><p className="font-semibold text-white">{u.name}</p><p className="text-xs text-white/40">{u.email}</p></div>
                  </div>
                </td>
                <td className="px-4 py-3 text-white/60">{u.phone || "—"}</td>
                <td className="px-4 py-3 text-white/60">{u._count?.orders ?? 0}</td>
                <td className="px-4 py-3 font-bold text-white">₹{Number(u.totalSpent || 0).toFixed(2)}</td>
        ...
        {Math.ceil(total / perPage) > 1 && (
          <div className="flex justify-between items-center px-4 py-3 border-t border-white/10">
            <p className="text-xs text-white/30">Page {page} of {Math.ceil(total / perPage)}</p>
