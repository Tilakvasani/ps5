  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-black" style={{ color: "#F8F8F8" }}>Users</h1>
          <p className="text-sm mt-1" style={{ color: "#F8F8F8" }}>{total} registered users</p>
        </div>
      </div>
      ...
      <div className="card p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[#0C1E39]">
            {loading ? Array.from({ length: 8 }).map((_, i) => <tr key={i}><td colSpan={6} className="px-4 py-3"><div className="h-4 rounded animate-pulse" style={{ background: "#0C1E39" }} /></td></tr>)
              : users.map(u => (
              <tr key={u.id} className="transition-colors"
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs" style={{ color: "#F8F8F8" }}>{u.email}</p>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3" style={{ color: "#F8F8F8" }}>{u.phone || "—"}</td>
                <td className="px-4 py-3" style={{ color: "#F8F8F8" }}>{u._count?.orders ?? 0}</td>
                ...
                <td className="px-4 py-3 text-xs" style={{ color: "#F8F8F8" }}>{new Date(u.createdAt).toLocaleDateString("en-IN")}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {Math.ceil(total / perPage) > 1 && (
          <div className="flex justify-between items-center px-4 py-3" style={{ borderTop: "1.5px solid #0C1E39" }}>
            <p className="text-xs" style={{ color: "#F8F8F8" }}>Page {page} of {Math.ceil(total / perPage)}</p>
