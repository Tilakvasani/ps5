  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-black" style={{ color: "#F8F8F8" }}>Products</h1>
          <p className="text-sm mt-1" style={{ color: "#F8F8F8" }}>{total} total products</p>
        </div>
      ...
      <div className="card p-0 overflow-hidden">
            ...
            <tbody className="divide-y divide-[#0C1E39]">
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i}><td colSpan={8} className="px-4 py-3"><div className="h-4 rounded animate-pulse" style={{ background: "#0C1E39" }} /></td></tr>
                ))
              ) : products.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-12 text-center" style={{ color: "#F8F8F8" }}>No products found</td></tr>
              ) : products.map(p => (
                <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="transition-colors"
                  ...
                  <td className="px-4 py-3">
                    <div className="h-10 w-10 rounded-lg overflow-hidden" style={{ background: "#0C1E39", border: "1px solid #0C1E39" }}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    {p.brand && <p className="text-xs" style={{ color: "#F8F8F8" }}>{p.brand}</p>}
                  </td>
                  <td className="px-4 py-3 font-mono text-xs" style={{ color: "#F8F8F8" }}>{p.sku}</td>
                  <td className="px-4 py-3" style={{ color: "#F8F8F8" }}>{p.category?.name || "—"}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => toggleActive(p)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${p.isActive ? "bg-[#FF5C00]" : "bg-[#0C1E39]"}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <Link href={`/products/${p.slug}`} target="_blank">
                        <button className="h-8 w-8 flex items-center justify-center rounded-lg transition-all" style={{ color: "#F8F8F8" }}
                          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = "#FFFFFF"; (e.currentTarget as HTMLElement).style.background = "#0C1E39"; }}
                          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = "#F8F8F8"; (e.currentTarget as HTMLElement).style.background = "transparent"; }}><Eye size={14} /></button>
                      </Link>
                      <Link href={`/admin/products/${p.id}`}>
                        <button className="h-8 w-8 flex items-center justify-center rounded-lg transition-all" style={{ color: "#F8F8F8" }}
                          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = "var(--or)"; (e.currentTarget as HTMLElement).style.background = "#0C1E39"; }}
                          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = "#F8F8F8"; (e.currentTarget as HTMLElement).style.background = "transparent"; }}><Edit3 size={14} /></button>
                      </Link>
                      <button onClick={() => handleDelete(p.id, p.name)}
                        className="h-8 w-8 flex items-center justify-center rounded-lg transition-all" style={{ color: "#F8F8F8" }}
                        ...
                        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = "#F8F8F8"; (e.currentTarget as HTMLElement).style.background = "transparent"; }}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3" style={{ borderTop: "1.5px solid #0C1E39" }}>
            <p className="text-xs" style={{ color: "#F8F8F8" }}>Showing {(page - 1) * perPage + 1}–{Math.min(page * perPage, total)} of {total}</p>
            <div className="flex gap-1">
              {Array.from({ length: Math.min(totalPages, 7) }).map((_, i) => (
                <button key={i} onClick={() => setPage(i + 1)}
                  className="h-8 w-8 rounded-lg text-xs font-semibold transition-all"
                  style={{ background: page === i + 1 ? "var(--or)" : "transparent", color: page === i + 1 ? "#FFFFFF" : "#F8F8F8" }}>
                  {i + 1}
                </button>
              ))}
            </div>
