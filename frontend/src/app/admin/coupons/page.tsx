  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-display font-black text-white">Coupons</h1>
      ...
      <div className="card p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {loading ? Array.from({ length: 5 }).map((_, i) => <tr key={i}><td colSpan={8} className="px-4 py-3"><div className="h-4 rounded bg-white/5 animate-pulse" /></td></tr>)
              : coupons.map(c => (
              <tr key={c.id} className="hover:bg-white/5 transition-colors">
                <td className="px-4 py-3 font-mono font-bold text-pink-400">{c.code}</td>
                <td className="px-4 py-3 text-white/60 capitalize">{c.discountType}</td>
                <td className="px-4 py-3 font-bold text-white">{c.discountType === "percent" ? `${c.discountValue}%` : `₹${c.discountValue}`}</td>
                <td className="px-4 py-3 text-white/50">₹{c.minOrderValue}</td>
                <td className="px-4 py-3 text-white/50">{c.usedCount || 0}{c.usageLimit ? `/${c.usageLimit}` : ""}</td>
                <td className="px-4 py-3 text-white/40 text-xs">{c.validTo ? new Date(c.validTo).toLocaleDateString("en-IN") : "—"}</td>
                ...
                <td className="px-4 py-3 flex gap-1">
                  <button onClick={() => openEdit(c)} className="h-8 w-8 flex items-center justify-center rounded-lg text-white/30 hover:text-blue-400 hover:bg-blue-400/10 transition-all"><Edit3 size={14} /></button>
                  <button onClick={() => handleDelete(c.id)} className="h-8 w-8 flex items-center justify-center rounded-lg text-white/30 hover:text-red-400 hover:bg-red-400/10 transition-all"><Trash2 size={14} /></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {modal !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-lg card p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="font-display font-bold text-white mb-5">{modal?.id ? "Edit" : "New"} Coupon</h2>
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <button type="button" onClick={() => setForm(f => ({ ...f, isActive: !f.isActive }))} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${form.isActive ? "bg-pink-500" : "bg-white/10"}`}>
