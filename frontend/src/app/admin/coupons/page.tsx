  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-black" style={{ color: "#F8F8F8", letterSpacing: "-0.04em" }}>Coupons</h1>
      ...
      <div className="zcard p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y" style={{ borderColor: "#0C1E39" }}>
            {loading ? Array.from({ length: 5 }).map((_, i) => <tr key={i}><td colSpan={8} className="px-4 py-3"><div className="h-4 rounded animate-pulse" style={{ background: "#0C1E39" }} /></td></tr>)
              : coupons.map(c => (
              <tr key={c.id} className="transition-colors" style={{ color: "#FFFFFF" }} onMouseEnter={e => e.currentTarget.style.background = "#0C1E39"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                <td className="px-4 py-3 font-mono font-bold" style={{ color: "var(--or)" }}>{c.code}</td>
                <td className="px-4 py-3 capitalize" style={{ color: "#F8F8F8" }}>{c.discountType}</td>
                <td className="px-4 py-3 font-bold" style={{ color: "#FFFFFF" }}>{c.discountType === "percent" ? `${c.discountValue}%` : `₹${c.discountValue}`}</td>
                <td className="px-4 py-3" style={{ color: "#F8F8F8" }}>₹{c.minOrderValue}</td>
                <td className="px-4 py-3" style={{ color: "#F8F8F8" }}>{c.usedCount || 0}{c.usageLimit ? `/${c.usageLimit}` : ""}</td>
                <td className="px-4 py-3 text-xs" style={{ color: "#F8F8F8" }}>{c.validTo ? new Date(c.validTo).toLocaleDateString("en-IN") : "—"}</td>
                <td className="px-4 py-3"><span className={`zbadge ${c.isActive ? "zbadge-gr" : "zbadge-rd"}`}>{c.isActive ? "Active" : "Inactive"}</span></td>
                <td className="px-4 py-3 flex gap-1">
                  <button onClick={() => openEdit(c)} className="h-8 w-8 flex items-center justify-center rounded-lg transition-all" style={{ color: "#F8F8F8" }} onMouseEnter={e => { e.currentTarget.style.color = "var(--or)"; e.currentTarget.style.background = "#0C1E39"; }} onMouseLeave={e => { e.currentTarget.style.color = "#F8F8F8"; e.currentTarget.style.background = "transparent"; }}><Edit3 size={14} /></button>
                  <button onClick={() => handleDelete(c.id)} className="h-8 w-8 flex items-center justify-center rounded-lg transition-all" style={{ color: "#F8F8F8" }} onMouseEnter={e => { e.currentTarget.style.color = "#EF4444"; e.currentTarget.style.background = "rgba(239, 68, 68, 0.1)"; }} onMouseLeave={e => { e.currentTarget.style.color = "#F8F8F8"; e.currentTarget.style.background = "transparent"; }}><Trash2 size={14} /></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {modal !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(5, 17, 36, 0.7)" }}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-lg zcard p-6 max-h-[90vh] overflow-y-auto" style={{ background: "#0C1E39", border: "1.5px solid #0C1E39" }}>
            <h2 className="font-bold mb-5" style={{ color: "#F8F8F8" }}>{modal?.id ? "Edit" : "New"} Coupon</h2>
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <button type="button" onClick={() => setForm(f => ({ ...f, isActive: !f.isActive }))} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${form.isActive ? "bg-[var(--or)]" : "bg-[#051124]"}`} style={{ border: "1.5px solid #0C1E39" }}>
