  ...
  return (
    <main className="min-h-screen bg-[#050505] flex items-center justify-center px-6">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 h-[400px] w-[400px] rounded-full bg-pink-500/8 blur-[100px]" />
      </div>
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-pink-500 to-yellow-400 mb-4">
            <Package2 size={28} className="text-black" />
          </div>
          <h1 className="text-3xl font-display font-black text-white">Admin Panel</h1>
          <p className="text-white/40 mt-1 text-sm">Zupwell Dashboard</p>
        </div>
        <div className="card p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.button type="submit" disabled={loading} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="btn-primary w-full py-3 disabled:opacity-50">
              {loading ? <span className="flex items-center justify-center gap-2"><span className="h-4 w-4 rounded-full border-2 border-white border-t-transparent animate-spin" />Signing in...</span> : "Sign In to Dashboard"}
            </motion.button>
          </form>
        </div>
      </motion.div>
