  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-black" style={{ color: "#F8F8F8", letterSpacing: "-0.04em" }}>Notifications</h1>
          {unread > 0 && <p className="text-sm mt-1" style={{ color: "#F8F8F8" }}>{unread} unread</p>}
        </div>
        {unread > 0 && (
      ...
      <div className="space-y-2">
        {loading ? Array.from({ length: 6 }).map((_, i) => <div key={i} className="zcard h-16 animate-pulse" style={{ background: "#0C1E39", borderColor: "#0C1E39" }} />) :
          notifications.length === 0 ? (
            <div className="zcard text-center py-16" style={{ color: "#F8F8F8" }}>
            ...
            <div key={n.id} onClick={() => !n.isRead && markRead(n.id)}
              ...
              style={{
                background: !n.isRead ? "rgba(255, 92, 0, 0.08)" : "#0C1E39",
                borderColor: !n.isRead ? "var(--or)" : "#0C1E39",
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm mt-0.5" style={{ color: "#F8F8F8" }}>{n.message}</p>
                <p className="text-xs mt-1" style={{ color: "#F8F8F8" }}>{new Date(n.createdAt).toLocaleString("en-IN")}</p>
              </div>
