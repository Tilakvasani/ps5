...
function ProductsContent() {
  ...
  return (
    <div className="pt-20">
      {/* Header */}
      <div className="relative py-12 px-6" style={{ borderBottom: "1.5px solid #1E2D4A" }}>
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div className="absolute -top-20 left-1/4 h-[300px] w-[300px] rounded-full" style={{ background: "rgba(255,92,0,0.08)" }} />
        </div>
        <div className="relative mx-auto max-w-7xl">
          <h1 className="text-4xl font-black mb-2" style={{ color: "#627d98" }}>All <span className="gradient-text">Products</span></h1>
          <p style={{ color: "#8F9CAE" }}>{total} products available</p>
        </div>
      </div>
        ...
        <aside className={`lg:w-64 flex-shrink-0 ${filterOpen ? "block" : "hidden lg:block"}`}>
          <div className="card sticky top-24 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="font-bold" style={{ color: "#627d98" }}>Filters</h2>
              <button onClick={clearFilters} className="text-xs" style={{ color: "var(--or)" }}>Clear all</button>
            </div>

            {/* Search */}
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: "#8F9CAE" }}>Search</label>
            ...
            {/* Category */}
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: "#8F9CAE" }}>Category</label>
              <div className="space-y-1">
                <button onClick={() => { setSelectedCat(""); setPage(1); }}
                  className="w-full text-left px-3 py-2 rounded-lg text-sm transition-all"
                  style={!selectedCat
                    ? { background: "rgba(255,92,0,0.15)", color: "var(--or)" }
                    : { color: "#8F9CAE" }
                  }>
                  All Categories
                </button>
          ...
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {Array.from({ length: 9 }).map((_, i) => (
                <div key={i} className="rounded-2xl aspect-[3/4] animate-pulse" style={{ border: "1.5px solid #1E2D4A", background: "#0C1E3E" }} />
              ))}
            </div>
          ) : products.length === 0 ? (
            <div className="text-center py-24" style={{ color: "#8F9CAE" }}>
              <p className="text-2xl mb-2">No products found</p>
              <button onClick={clearFilters} className="text-sm" style={{ color: "var(--or)" }}>Clear filters</button>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {Array.from({ length: totalPages }).map((_, i) => (
                <motion.button key={i} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  ...
                  style={page === i + 1
                    ...
                    : { border: "1.5px solid #1E2D4A", color: "#8F9CAE" }
                  }>
                  {i + 1}
                </motion.button>
              ))}
