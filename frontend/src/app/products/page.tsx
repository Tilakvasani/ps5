    ...
    <div className="pt-20">
      ...
      <div className="relative border-b border-[#E8E2D9] py-12 px-6">
        ...
        <div className="relative mx-auto max-w-7xl">
          <h1 className="text-4xl font-black text-[#001c54] mb-2">All <span className="gradient-text">Products</span></h1>
        ...
        <aside className={`lg:w-64 flex-shrink-0 ${filterOpen ? "block" : "hidden lg:block"}`}>
          <div className="card sticky top-24 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-[#001c54]">Filters</h2>
            ...
            <div>
              ...
              <div className="space-y-1">
                <button onClick={() => { setSelectedCat(""); setPage(1); }}
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-all ${!selectedCat ? "bg-[#EB9220]/20 text-[#EB9220]" : "text-[#45353E] hover:text-[#001c54] hover:bg-[#FCFAF6]"}`}>
                  All Categories
                </button>
                {categories.map((cat) => (
                  <button key={cat.id} onClick={() => { setSelectedCat(cat.slug); setPage(1); }}
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-all ${selectedCat === cat.slug ? "bg-[#EB9220]/20 text-[#EB9220]" : "text-[#45353E] hover:text-[#001c54] hover:bg-[#FCFAF6]"}`}>
                    {cat.name}
                  </button>
                ))}
              </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {loading ? (
            ...
            <div className="text-center py-24 text-[#45353E]">
              ...
              <button onClick={clearFilters} className="text-[#EB9220] hover:text-[#EB9220] text-sm">Clear filters</button>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="flex justify-center gap-2 mt-12">
              {Array.from({ length: totalPages }).map((_, i) => (
                <motion.button key={i} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  onClick={() => setPage(i + 1)}
                  className={`h-9 w-9 rounded-lg text-sm font-semibold transition-all ${page === i + 1 ? "bg-[#EB9220] text-white" : "border border-[#E8E2D9] text-[#45353E] hover:border-[#E8E2D9] hover:text-[#001c54]"}`}>
                  {i + 1}
                </motion.button>
              ))}
            </div>
