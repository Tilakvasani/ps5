...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className={`border border-[#D9DEE8] rounded-xl overflow-hidden transition-all duration-200 ${open ? "border-[#F47C41]/40 shadow-sm" : "hover:border-[#F47C41]/20"}`}>
      <button onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between gap-4 px-5 py-4 text-left">
        <span className={`font-semibold text-sm md:text-base transition-colors ${open ? "text-[#F47C41]" : "text-[#111827]"}`}>
          {q}
        </span>
        <motion.div animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }} className="shrink-0">
          <ChevronDown size={18} className={open ? "text-[#F47C41]" : "text-[#9CA3AF]"} />
        </motion.div>
      </button>
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="px-5 pb-5 text-sm text-[#374151] leading-relaxed border-t border-[#F4F6FA] pt-3">
              {a}
            </div>
          </motion.div>
        )}
  ...
  return (
    <main className="min-h-screen bg-[#F4F6FA] overflow-x-hidden">
      ...
      <section className="relative pt-32 pb-16 px-6 bg-white overflow-hidden">
        <div className="pointer-events-none absolute -top-40 -right-40 h-[400px] w-[400px] rounded-full bg-[#F47C41]/6 blur-[100px]" />
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <motion.h1 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="text-5xl md:text-6xl font-display font-black text-[#111827] mb-4 leading-tight">
      ...
      {/* ── Category Filter ── */}
      <section className="py-8 px-6 bg-white border-b border-[#D9DEE8]">
          ...
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`text-sm px-4 py-2 rounded-full font-semibold transition-all ${
                activeCategory === cat
                  ? "bg-[#F47C41] text-white"
                  : "bg-[#F4F6FA] text-[#6B7280] hover:bg-[#F47C41]/10 hover:text-[#F47C41]"
              }`}>
              {cat}
            </button>
          ))}
          ...
          {filtered.map((section, i) => (
            <motion.div key={section.category}
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-xl font-display font-black text-[#111827]">{section.category}</h2>
              </div>
      ...
      <section className="py-16 px-6 bg-white">
        <div className="mx-auto max-w-xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-2xl font-display font-black text-[#111827] mb-2">
              Still have a question?
            </h2>
